class Site < ActiveRecord::Base
  ...
  scope :created_on, lambda { |day|
    active.tagged_with('adult', exclude: true).where(created_at: day).order('created_at desc')
  }
  scope :first_billable_plays_on, lambda { |day|
    active.tagged_with('adult', exclude: true).where(first_billable_plays_at: day).by_last_30_days_billable_video_views
  }
