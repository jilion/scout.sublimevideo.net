...
try {
  ...
  page.open(address, function (status) {
    if (status !== 'success') {
      ...
      if (safe_status == 'not_safe') {
        var pornKeywordsCount = countPornKeywords(page);

        if (pornKeywordsCount >= 10) {
          console.log('Porn keywords for ' + address + ' : ' + pornKeywordsCount);
...
function countPornKeywords(page) {
  return page.evaluate(function () {
    ...
    var pornRegex = /(\W|^)(porn|pr0n|boobs|sex|hentai|bukkake|bdsm|pov|teen|gay|pussy|nude|nudism|naked|breast|cock|dick|erotic|virgin|amateure|mature|cumming|facial|horny|orgasm|wife|anal|fetish)(\W|$)/gi
    ...
    if (pornKeywordsInHead !== null) {
      console.log('Porn keywords in head: ' + pornKeywordsInHead);
      pornKeywordsCount += pornKeywordsInHead.length
    }
    if (pornKeywordsInBody !== null) {
      console.log('Porn keywords in body: ' + pornKeywordsInBody);
